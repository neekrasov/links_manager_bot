...
async def simple_request(url: str, any_id: list=None) -> dict:
    if any_id is None:
        any_id = []
...
async def get_user(user_id: int) -> dict:
    return await simple_request(url=user_url, any_id=[user_id])


async def get_group(group_id: int) -> dict:
    return await simple_request(url=group_url, any_id=[group_id])


async def get_link(link_id: int) -> dict:
    return await simple_request(url=link_url, any_id=[link_id])


async def get_users() -> dict:
    return await simple_request(url=users_url)


async def get_groups() -> dict:
    return await simple_request(url=groups_url)


async def get_links() -> dict:
    return await simple_request(url=links_url)


async def get_datetime_for_all_links() -> dict:
    return await simple_request(url=links_datetime)


async def get_datetime_for_link(link_id: int) -> list:
    return await simple_request(url=link_datetime, any_id=[link_id])
...
async def get_groups_for_user(user_id: int) -> dict:
    return await simple_request(url=groups_for_user_url, any_id=[user_id])


async def get_links_for_group(group_id: int) -> dict:
    return await simple_request(url=links_for_group_url, any_id=[group_id])
